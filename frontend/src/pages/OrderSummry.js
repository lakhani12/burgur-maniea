...
const OrderSummry = () => {
  ...
  const orderType = useSelector(state => state.order.orderType) || 'Take Away'
  ...
  const handlePlaceOrder = () => {
    dispatch(placeOrder({ orderItems: cartItems, shippingAddress: address, paymentMethod: paymentMethod, itemsPrice: cartPrice, totalprice: totalPrice, shippingPrice: deleviryPrice, email: user.eamil, userName: user.name, orderType }))
  }

  const PaymentHandler = (paymentResult) => {
    const { razorpay_payment_id } = paymentResult
    dispatch(placeOrder({ orderItems: cartItems, shippingAddress: address, paymentMethod: paymentMethod, itemsPrice: cartPrice, totalprice: totalPrice, shippingPrice: deleviryPrice, email: user.eamil, userName: user.name, paymentId: razorpay_payment_id, orderType }))
